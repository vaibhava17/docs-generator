...
} from "../lib/github-docs-generator";
...
program
  ...
  .option(
    "-p, --path <path>",
    "Target directory path to document (default: entire repo)"
  )
  .option(
    "-b, --branch <branch>",
    "Branch name for documentation (default: docs-generation)"
  )
  .option("--provider <provider>", "AI provider: openai or gemini", "openai")
  .option(
    "--api-key <key>",
    "AI API key (or set OPENAI_API_KEY/GEMINI_API_KEY env var)"
  )
  .option("--overwrite", "Overwrite existing documentation files")
  .option("-f, --force", "Skip confirmation prompts")
  .option(
    "-l, --list-files",
    "List files that would be documented without generating docs"
  )
...
async function handleGenerate(
  ...
  if (!apiKey) {
    throw new Error(
      `${options.provider.toUpperCase()} API key not found. Use --api-key option or set ${options.provider.toUpperCase()}_API_KEY environment variable`
    );
  }
...
async function handleListFiles(
  ...
  try {
    ...
    if (totalFiles > 0) {
      ...
      console.log(
        `💡 Run without --list-files to generate documentation for ${sourceFiles.length} files`
      );
    } else {
