...
export async function POST(request: NextRequest) {
  try {
    ...
    const { repoUrl, branchName, apiKey } = formData;
    ...
    updateStatus({
      status: "error",
      message:
        "Failed to start documentation generation: " + (error as Error).message,
    });
    return NextResponse.json(
...
async function generateDocumentationAsync(formData: {
  ...
  console.log(
    "🔍 Starting generation with token:",
    githubToken ? "***provided***" : "NOT PROVIDED"
  );
  ...
  try {
    ...
    for (let i = 0; i < sourceFiles.length; i++) {
      ...
      try {
        ...
        failedFiles.push(filePath);
        updateStatus({
          message: `❌ Error documenting ${filePath}: ${
    ...
    await generator.commitAndPush(branchName, {
      url: repoUrl,
      token: githubToken,
    });
    ...
    if (failedFiles.length > 0) {
      updateStatus({
        message: `⚠️ ${
          failedFiles.length
        } files failed to process: ${failedFiles.slice(0, 3).join(", ")}${
